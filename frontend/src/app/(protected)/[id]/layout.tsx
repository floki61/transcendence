				...
				<div className='h-full w-full flex flex-col gap-16'>
					<div className='flex gap-4 h-[22%]'>
						...
						<div className='w-3/5 h-full flex flex-col justify-between px-4'>
								...
								<div className='flex justify-between'>
									...
									<div className='cursos-pointer w-[42%]'>
										{params.id !== user.user?.id && friend.isfriend === "friend" && (
											<div className='flex justify-end w-full' onClick={() => {SetOptions(!options)}}>
												{!options && (
							...
							{params.id !== user.user?.id && friend.isfriend === "friend" && !request && !accept && (
								<div className='flex justify-end gap-3'>
									<ProfileButton color="bg-primecl" text="Friends" icon={MdPeopleAlt} classname='w-1/5 mb-2'/>
									<ProfileButton color="bg-[#6A6666]" text="Unfriend" icon={FaUserMinus} action={Unfriend} classname='w-1/5 mb-2'/>
								</div>
							)}
