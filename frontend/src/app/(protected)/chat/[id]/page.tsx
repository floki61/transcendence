...
const Convo = ({ params }: { params: any }) => {
  ...
  if (visible === "PROTECTED" && id !== user.user?.id) {
    return (
      <div className="p-32 h-full w-full">
        <div className="bg-quatrocl rounded-xl h-full w-full p-4 flex flex-col items-center justify-evenly">
            <h1 className="text-segundcl text-2xl text-center px-8">This room is protected, you need to enter the password in order to join the room</h1>
            <input
              ...
              {user.user && chat && chat.map((chatie) => (
                ...
                >
                  <Chatmsg
                    ...
                    className={`flex justify-between ${(user.user?.id === chatie.user?.uid) || (user.user?.id === chatie.uid) ? "self-end bg-primecl rounded-s-lg rounded-br-lg" : "bg-quatrocl rounded-e-lg rounded-bl-lg self-start"}`}
                  />
                </div>
              ))}
            </div>
